...
class WP_AI_Workflows_Chat_Preview {
	...
	public function render_preview_page() {
		...
		<body>
			...
			<script>
				// Set up settings BEFORE loading the main app
				window.wpAiWorkflowsSettings = {
					apiUrl: '<?php echo esc_js( rest_url( 'wp-ai-workflows/v1' ) ); ?>',
					nonce: '<?php echo esc_js( wp_create_nonce( 'wp_rest' ) ); ?>',
					siteUrl: '<?php echo esc_js( get_site_url() ); ?>',
					assetsUrl: '<?php echo esc_js( plugins_url( 'assets', WP_AI_WORKFLOWS_PLUGIN_DIR . 'wp-ai-workflows.php' ) ); ?>',
					isChat: true,
					isPreview: true
				};
